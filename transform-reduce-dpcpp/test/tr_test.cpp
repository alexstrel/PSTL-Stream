...
//internal queue: 
  ...
  reduce_t r = impl::transform_reduce(p, 0, n, n, 0.0f, ONEAPI::plus<data_t>(), fn_ref);
#else
  auto compute_axpyDot_ = [=, x_ = x.data(), y_ = y.data()] (const int i, const int j = 0) {y_[i] = a*x_[i]+y_[i]; return (y_[i]*x_[i]);};

  reduce_t r = impl::transform_reduce(p, 0, n, n, 0.0f, ONEAPI::plus<data_t>(), compute_axpyDot_);
#endif
